...
def connect():
    ...
    cur = conn.cursor()
    cur.execute(
        "CREATE TABLE IF NOT EXISTs hotel (id INTEGER PRIMARY KEY , name TEXT , address TEXT , phone_number INTEGER, no_of_days INTEGER , room_type TEXT , total INTEGER)")
    conn.commit()
    conn.close()
...
def search(name="", address="", phone_number="", room_type="", no_of_days="", total=""):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM hotel WHERE name=? OR address=? OR phone_number=?  OR  room_type=?  OR  no_of_days=?  OR  total=?",
